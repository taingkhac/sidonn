...
export function ContentManager() {
  ...
  const filtered = blocks.filter(b => {
    const matchesSearch = b.block_key.toLowerCase().includes(search.toLowerCase()) || 
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        {loading ? (
          ...
          filtered.map((block) => (
            <div key={block.id} className="bg-white p-4 rounded-xl border group hover:border-primary/50 transition-all shadow-sm">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Layout className="h-4 w-4 text-slate-400" />
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">{block.view_target}</span>
                  </div>
                    ...
                    {LANGUAGES.slice(0, 2).map(lang => (
                      <div key={lang} className="text-sm flex gap-2">
                        ...
                        <span className="text-slate-600 line-clamp-1 italic">
                          {block.translations[lang] || <span className="text-red-300">Chưa dịch</span>}
                        </span>
                      </div>
                    ))}
                  </div>
                </div>
                <Button variant="ghost" size="icon" onClick={() => handleEdit(block)} className="opacity-0 group-hover:opacity-100 transition-all">
                ...
                {LANGUAGES.map(lang => (
                  <div key={lang} className="space-y-2 p-3 bg-slate-50 rounded-lg border">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-bold bg-slate-200 px-2 py-0.5 rounded uppercase">{lang}</span>
                    </div>
