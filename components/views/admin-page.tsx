import { ArrowLeft, Users, FileText, Settings, BarChart3, Database, LayoutDashboard, Menu as MenuIcon, Type, FileEdit, Lock, Mail } from 'lucide-react'
...
export function AdminPage({ onNavigate }: AdminPageProps) {
  ...
  useEffect(() => {
    ...
    async function fetchStats() {
      try {
        const [{ count: artCount }, { count: blockCount }, { count: menuCount }, { count: leadCount }] = await Promise.all([
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault()
    if (pin === '1234') { // Mã PIN mặc định, bạn có thể đổi thành biến môi trường
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-12 text-lg">Đăng nhập</Button>
          </form>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-slate-600 leading-relaxed">
                      ...
                      Số lượng bài viết và khối nội dung được thống kê trực tiếp từ bảng lưu trữ Supabase.
                    </p>
                    <div className="p-4 bg-slate-50 rounded italic text-xs text-slate-500">
                      Lưu ý: Mọi thay đổi về slug bài viết sẽ ảnh hưởng trực tiếp đến URL SEO của trang client.
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-primary/5 p-8 rounded-xl border border-primary/10 flex flex-col justify-center">
                   ...
                   <p className="text-slate-600 mb-6">Mọi thứ đang hoạt động ổn định. Bạn có thể bắt đầu quản trị các nội dung cho website Sidonn ngay bây giờ.</p>
                   <div className="flex gap-3">
                      <button onClick={() => setActiveTab('content')} className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium shadow-md hover:bg-primary/90 transition-all">Sửa trang chủ</button>
                      <button onClick={() => setActiveTab('articles')} className="px-4 py-2 bg-white text-slate-900 border border-slate-200 rounded-lg text-sm font-medium hover:bg-slate-50 transition-all">Viết bài mới</button>
                   </div>
                </div>
              </div>
            </div>
