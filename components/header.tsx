...
export function Header({ onNavigate }: HeaderProps) {
  ...
  const currentSolutionsMenu = React.useMemo(() => {
    if (rawMenus.length > 0) {
      ...
      return categories.map(cat => ({
        ...
        items: items.filter(item => item.parent_id === cat.id).map(item => ({
  ...
  const currentProductsMenu = React.useMemo(() => {
    if (rawMenus.length > 0) {
      return rawMenus.filter(m => m.menu_type === 'products').map(m => ({
  ...
  const currentSoftwareMenu = React.useMemo(() => {
    if (rawMenus.length > 0) {
      return rawMenus.filter(m => m.menu_type === 'software').map(m => ({
          name: m.translations[lang] || m.translations['en'] || m.key,
          view: m.view_target
      }))
    }
  ...
  const currentCompanyMenu = React.useMemo(() => {
    if (rawMenus.length > 0) {
      return rawMenus.filter(m => m.menu_type === 'company').map(m => ({
          name: m.translations[lang] || m.translations['en'] || m.key,
          view: m.view_target
      }))
    }
        ...
        <div className="container mx-auto px-4">
          <div className="flex h-10 items-center justify-between text-sm">
            <div className="flex items-center gap-6">
              <button onClick={() => onNavigate?.('newsroom')} className="text-muted-foreground hover:text-foreground transition-colors">
                {translate('nav_newsroom', lang, 'Newsroom')}
              </button>
              <button onClick={() => onNavigate?.('support')} className="text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" className="hidden lg:flex hover:bg-secondary transition-all duration-200 hover:scale-110">
            ...
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              <SheetTrigger asChild className="lg:hidden">
                <Button variant="ghost" size="icon" className="hover:bg-secondary transition-all duration-200 hover:scale-110">
                ...
                <nav className="flex flex-col gap-4 mt-8">
                  <Accordion type="single" collapsible className="w-full">
                    <AccordionItem value="solutions">
                      <AccordionTrigger>{translate('nav_solutions', lang, 'Solutions')}</AccordionTrigger>
                      <AccordionContent>
                    ...
                    <AccordionItem value="products">
                      <AccordionTrigger>{translate('nav_products', lang, 'Products')}</AccordionTrigger>
                      <AccordionContent>
                        <ul className="space-y-2">
                          {currentProductsMenu.map((item) => (
                            <li key={item.name}>
                    ...
                    <AccordionItem value="software">
                      <AccordionTrigger>{translate('nav_software', lang, 'Software')}</AccordionTrigger>
                      <AccordionContent>
                        <ul className="space-y-2">
                          {currentSoftwareMenu.map((item) => (
                            <li key={item.name}>
                    ...
                    <AccordionItem value="company">
                      <AccordionTrigger>{translate('nav_company', lang, 'Company')}</AccordionTrigger>
                      <AccordionContent>
                        <ul className="space-y-2">
                          {currentCompanyMenu.map((item) => (
                            <li key={item.name}>
                  ...
                  <div className="pt-4 border-t space-y-3">
                    <Button variant="ghost" className="w-full justify-start transition-all duration-200 hover:bg-secondary hover:translate-x-1" size="sm">
