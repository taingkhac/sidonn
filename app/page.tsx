...
type ViewType = 'home' | 'solutions-overview' | 'resources' | 'admin-dashboard' | 'contact' | 
  'smart-warehouse' | 'fulfillment' | 'sorting' | 'automation' | 'catering' | 'seaport-agv' |
  'about' | 'career' | 'newsroom' | 'support' | 'privacy' | 'terms' |
  'product-racking' | 'product-containers' | 'product-conveyors' | 'product-robotics' |
  'software-wamas' | 'software-sap' | 
  'news-expansion-asia-pacific' | 'news-robotics-fortune-500' | 'news-partnership-ecommerce'
...
export function PageContent() {
  ...
  useEffect(() => {
    ...
    const nonDbViews = ['home', 'solutions-overview', 'admin-dashboard', 'contact', 'about', 'career', 'newsroom', 'support', 'privacy', 'terms', 'resources']
  ...
  const renderView = () => {
    ...
    if (dynamicArticle) {
      ...
      const mappedArticle = {
        ...
        applications: art.applications_translations?.[lang] || art.applications_translations?.['en'] || [],
      ...
      return <SolutionDetailPage solution={mappedArticle as any} onNavigate={handleNavigate} isHtml />
    }
